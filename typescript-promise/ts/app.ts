...
(function () {
    ...
    Backendless.UserService.isValidLogin<Promise<boolean>>().then(startApp).catch(onLoginNotValid);

    function onLoginNotValid():void {
        //if current user login is not valid we need to logout the user and create a new one
        Backendless.UserService.logout<Promise<boolean>>().then(function () {
    ...
    function createAndLoginUser():void {
        ...
        //register a new user
        Backendless.UserService.register<Promise<AppUser>>(user).then(function () {
            //login new created user and keep it logged, event if you refreshed browser page the user stay logged
            Backendless.UserService.login<Promise<AppUser>>(userEmail, userPass, true).then(startApp);
        });
    }
